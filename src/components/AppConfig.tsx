					...
					{categories.map((category) => {
						...
						return (
							<button
								...
								className={`w-full px-3 py-2 rounded-lg flex items-center gap-2 text-left transition-colors ${isActive
				...
				{(activeCategory === 'General' || activeCategory === 'Appearance') && (
					<div className="rounded-xl bg-steam-800 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_14px_30px_rgba(0,0,0,0.2)]">
						...
						<div className="mt-4 rounded-lg bg-steam-900/45 px-4 py-3 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.06)]">
							...
							{credentialsStatus && (
								<div
									className={`mt-3 rounded-md px-3 py-2 text-sm ${credentialsStatus.type === 'success'
							...
							<div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
								{hoverEffectOptions.map((option) => (
									<button
										...
										className={`px-3 py-2 rounded-lg flex flex-col items-center gap-1.5 transition-all ${option.value === 'zoom' ? 'hover:scale-105' :
										...
										title={option.label}
									>
										<span className={`${option.value === 'zoom' ? 'text-sky-400' :
							...
							<div className="flex items-center justify-between gap-4">
								...
								<div className="flex items-center mr-2">
									<div
										...
										className={`relative inline-flex h-8 w-16 items-center select-none rounded-md p-1 transition-all duration-300 focus:outline-none ${isUpdatingRunOnStartup
												? 'cursor-not-allowed opacity-50 grayscale bg-zinc-600'
												: 'cursor-pointer '
											} ${runOnStartup ? 'bg-sky-400' : 'bg-zinc-700'}`}
									>
										<div
											className={`h-6 w-6 bg-white rounded-md shadow transform transition-all duration-400 ${runOnStartup ? 'translate-x-8 rotate-90' : 'translate-x-0 rotate-0'
							...
							<div className="mt-4 flex items-center justify-between gap-4">
								...
								<div className="flex items-center mr-2">
									<div
										...
										className={`relative inline-flex h-8 w-16 items-center select-none rounded-md p-1 transition-all duration-300 focus:outline-none ${isUpdatingReduceWhilePlaying
												? 'cursor-not-allowed opacity-50 grayscale bg-zinc-600'
												: 'cursor-pointer '
						...
						<div className="mt-4 rounded-lg bg-steam-900/45 px-4 py-3 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.06)]">
							...
							{maintenanceStatus && (
								<div
									className={`mt-3 rounded-md px-3 py-2 text-sm ${maintenanceStatus.type === 'success'
											? 'bg-emerald-700/30 text-emerald-200 border border-emerald-500/40'
											: 'bg-red-700/30 text-red-200 border border-red-500/40'
										}`}
								>
